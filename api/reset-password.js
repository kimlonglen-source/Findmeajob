...
async function sendResetEmail(toEmail, toName, resetUrl) {
  ...
  if (!apiKey) {
    // No email service configured - return token for fallback
    return { sent: false };
  }
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:2rem;background:#0a0a08;color:#f2efe8;border-radius:12px">
      ...
      <p style="color:#888480;font-size:.8rem;line-height:1.6;margin-bottom:.5rem">This link expires in 1 hour. If you didn't request this, you can safely ignore this email.</p>
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (email && !token && !newPassword) {
      ...
      const resetUrl = `https://www.findmeajob.co.nz/employer-portal.html?reset=${resetToken}&email=${encodeURIComponent(email)}`;
      const emailResult = await sendResetEmail(email, emp.name, resetUrl);
      if (emailResult.sent) {
        return res.status(200).json({ success: true, method: 'email' });
      } else {
        // Fallback if Resend not configured - return token (dev mode only)
        return res.status(200).json({ success: true, method: 'link', resetUrl, token: resetToken });
      }
    }
    ...
    if (token && newPassword && email) {
      const raw = await hget('employers', email);
      if (!raw) return res.status(404).json({ error: 'Invalid reset link.' });
      const emp = typeof raw === 'string' ? JSON.parse(raw) : raw;
      if (emp.resetToken !== token) return res.status(400).json({ error: 'Invalid or expired reset link. Please request a new one.' });
      if (new Date(emp.resetExpiry) < new Date()) return res.status(400).json({ error: 'This reset link has expired. Please request a new one.' });
      emp.password = newPassword;
      delete emp.resetToken;
      delete emp.resetExpiry;
      emp.passwordChanged = new Date().toISOString();
      await hset('employers', email, emp);
