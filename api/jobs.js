module.exports = async function handler(req, res) {
  ...
  let url = `https://api.adzuna.com/v1/api/jobs/nz/search/1?app_id=${appId}&app_key=${appKey}&results_per_page=${perPage}&what=${encodeURIComponent(clean)}&sort_by=relevance`;
  ...
  try {
    ...
    if (!data.results || !data.results.length) {
      ...
      const r2 = await fetch(`https://api.adzuna.com/v1/api/jobs/nz/search/1?app_id=${appId}&app_key=${appKey}&results_per_page=${perPage}&what=${encodeURIComponent(word)}&sort_by=relevance`, { headers: { 'Accept': 'application/json' } });
