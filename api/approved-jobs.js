  ...
  if (req.query && req.query.blogPage === "1") {
    ...
    if (bPosts.length) {
      ...
      bPosts.forEach(function(p, idx) {
        ...
        bHtml += '<div style="display:flex;align-items:center;gap:.75rem;margin-bottom:.75rem;font-size:.75rem;color:var(--text3);font-weight:500"><span itemprop="author" itemscope itemtype="https://schema.org/Organization"><span itemprop="name">FindMeAJob.co.nz</span></span><span>&bull;</span><time itemprop="datePublished" datetime="'+dt+'">'+dt+'</time><span>&bull;</span><span>'+Math.ceil((bd.length||100)/1000)+' min read</span></div>';
